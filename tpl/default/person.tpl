<table width="100%">
    ...
    <tr>
        ...
        <td class="person_right">
            <div class="person_text">
                <!-- <?= date($conf['bdformat'], strtotime($contact->birthdate)) ?> -->
...
<!-- Begin Buttons -->
    ...
    <form method="POST" action="<?= $PHP_SELF ?>">
        ...
        <input type="hidden" name="id" value="<?= $contact->id ?>" />
        <input type="submit" value="<?= $lang['btn_delete']?>" onClick="return confirm('<?= $lang['confirm_delete'] ?>')" class="button" style="float: right;" />
    </form>
    <form method="post" action="<?= $PHP_SELF ?>">
