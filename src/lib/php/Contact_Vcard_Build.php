...
function vcard_addcslashes($string, $escapes) {
    $out_string = '';
...
class Contact_Vcard_Build extends PEAR {
    ...
    */
    ...
    function escape(&$text)
    {
        if (is_object($text)) {
            ...
            //cwacha: escaping is context sensitive!!
            $text = vcard_addcslashes($text, ":;,\n\\");
            /*
            // escape escapes!!
            $regex = '\\\\';
    ...
    */
    ...
    function getMeta($comp, $iter = 0)
    {
        ...
        // test for items
        if($this->hasItems($comp, $iter)) {
            $this->item++;
            $text = 'item' . $this->item . '.' . $text;
        }
        
        return $text;
    }
    
    function hasItems($comp, $iter)
    {
        foreach($this->value[$comp][$iter] as $key => $part) {
            if(!is_numeric($key)) return true;
        }
        return false;
    }
    
    function setItem($comp, $iter, $item, $text)
    {
        $this->setValue($comp, $iter, $item, $text);
    }
    
    function getItems($comp, $iter)
    {
        $lines = array();
        foreach($this->value[$comp][$iter] as $key => $part) {
            if(!is_numeric($key)) {
                $lines[] = 'item' . $this->item . '.' . $key . ':' . $this->getValue($comp, $iter, $key);
            }
        }
        return implode("\n", $lines);
    }
    ...
    */
    ...
    {
        $this->autoparam = 'URL';
        $iter = $this->countIter('URL');
        $this->setValue('URL', $iter, 0, $text);
    }
    ...
    */
    ...
    function getURL($iter = 0)
    {
        return $this->getMeta('URL', $iter) . $this->getValue('URL', $iter, 0);
    }
    ...
    function fetch()
    {
        ...
        if (isset($this->value['ADR'])) {
            foreach ($this->value['ADR'] as $key => $val) {
                ...
                $item = $this->getItems('ADR', $key);
                if(!empty($item)) $lines[] = $item;
            }
        }
        ...
        if (isset($this->value['URL'])) {
            foreach ($this->value['URL'] as $key => $val) {
                $lines[] = $this->getURL($key);
                $item = $this->getItems('URL', $key);
                if(!empty($item)) $lines[] = $item;
            }
        }
        ...
        // cwacha:
        // add all remaining fields (all fields except those in $types which we have already processed)
        $types = array( 'VERSION', 'N', 'FN', 'NAME', 'SOURCE', 'NICKNAME', 'PHOTO', 'BDAY', 'ADR', 'LABEL',
                        'TEL', 'EMAIL', 'MAILER', 'TZ', 'GEO', 'TITLE', 'ROLE', 'LOGO',
                        'AGENT', 'ORG', 'CATEGORIES', 'NOTE', 'PRODID', 'REV',
                        'SORT-STRING', 'SOUND', 'UID', 'URL', 'CLASS', 'KEY');

        foreach($this->value as $key => $val) {
            if(!in_array($key, $types)) {
                foreach($this->value[$key] as $iter => $iter_value) {
                    $lines[] = $this->getMeta($key, $iter) . $this->getValue($key, $iter, 0);
                    $item = $this->getItems($key, $iter);
                    if(!empty($item)) $lines[] = $item;
                }
            }
        }    
