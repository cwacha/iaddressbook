...
function img_convert($in_image, $type='png') {
    global $conf;
    ...
    if(!is_readable($conf['im_convert'])) return $out_image;

    $process = proc_open($conf['im_convert']." - $type:-", $desc_spec, $pipes);
    ...
    if(!empty($err)) {
        echo "<pre>";
        echo "ERROR: Could not convert image to $type<br>";
        echo "      Message: $err <br>";
        echo "      in_image size: ". strlen($in_image) . "<br>";
        echo "      out_image size: ". strlen($out) . "<br>";
        echo "      out_image: ". $out. "<br>";
        echo "</pre>";
    }
