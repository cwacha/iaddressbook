...
function act_importvcard() {
    ...
    foreach($data as $card) {
        ...
        if($AB->is_duplicate($contact)) {
            msg("$contact->firstname $contact->lastname is duplicate!!<br>", -1);
            ...
            if($AB->set($contact) == false) {
                $error++;
                msg("Could not import contact $contact->firstname $contact->lastname<br>", -1);
            } else $imported++;
        }
    }
    ...
    if($imported > 0) msg("$imported vCard(s) succesfully imported!<br>", 1);
    
    $n = count($data);
    if($n == 0) {
        msg("no vCards found to import.<br>");
        $error++;
    ...
    if($error != 0) {
        msg("Import ended with $error error(s).<br>");
    }    
...
function act_exportvcard_cat($action) {
    global $contactlist;
    global $conf;
    ...
    if(!empty($contactlist)) {
        // send the vcard
        $filename = "All Contacts (" . count($contactlist) . ")";
        $filename = trim($filename);
        $filename .= ".vcf";
...
function vcard2contact($card) {
    ...
    if(is_array($card['PHOTO'])) {
        ...
        if($enc == "BASE64" || $enc == "B") {
            ...
            msg("Cannot import image. Unknown encoding $enc<br>", -1);
        }
    }
