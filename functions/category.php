...
class categories {
    ...
    function find($person_id, $limit = 1000) {
        ...
        $lastimport = $db->Quote("__lastimport__");
		
		$sql  = "SELECT * FROM ".$db_config['dbtable_cat'].", ".$db_config['dbtable_catmap']." WHERE (";
        $sql .= $db_config['dbtable_cat'].".id = ".$db_config['dbtable_catmap'].".category_id ) AND "; 
		$sql .= "( person_id = $searchstring ) AND ( name != ".$lastimport." ) ";
