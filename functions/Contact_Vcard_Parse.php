...
class Contact_Vcard_Parse {
    ...
    */
    ...
    function fromText($text, $decode_qp = true, $encoding = 'ISO-8859-1')
    {
        ...
        //cwacha: assume input is utf-8 and convert it if necessary
        $enc = get_unicode($text);
        iconv_set_encoding("internal_encoding", "UTF-8");
        if($enc == false) {
            msg("Could not detect encoding of file. Trying with $encoding...<br>", -1);
            $text = iconv($encoding, 'UTF-8', $text);
        } else {
            msg("detected $enc encoding<br>");
            $text = iconv($enc, 'UTF-8', $text);
            if(strlen($text) == 0) msg("what the heck???<br>", -1);
