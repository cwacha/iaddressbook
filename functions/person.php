...
class person {
    ...
    function html_escape() {
        ...
        $this->firstname2 = htmlspecialchars($this->firstname2);
        $this->lastname = htmlspecialchars($this->lastname);
        $this->suffix = htmlspecialchars($this->suffix);
        $this->nickname = htmlspecialchars($this->nickname);
        $this->jobtitle = htmlspecialchars($this->jobtitle);
        $this->department = htmlspecialchars($this->department);
        $this->organization = htmlspecialchars($this->organization);
        $this->note = htmlspecialchars($this->note);
        
        $this->birthdate = htmlspecialchars($this->birthdate);
        
        $this->company = (boolean)((int)$this->company);
        $this->id = (int)$this->id;
        
        $this->creationdate = htmlspecialchars($this->creationdate);
        $this->modificationdate = htmlspecialchars($this->modificationdate);
        
        if(!is_array($this->addresses)) {
            foreach($this->addresses as $key => $value) { $this->addresses[$key] = htmlspecialchars($this->addresses[$value]); }
        }
        if(!is_array($this->emails)) {
            foreach($this->emails as $key => $value) { $this->emails[$key] = htmlspecialchars($this->emails[$value]); }
        }
        if(!is_array($this->phones)) {
            foreach($this->phones as $key => $value) { $this->phones[$key] = htmlspecialchars($this->phones[$value]); }
        }
        if(!is_array($this->chathandles)) {
            foreach($this->chathandles as $key => $value) { $this->chathandles[$key] = htmlspecialchars($this->chathandles[$value]); }
        }
        if(!is_array($this->relatednames)) {
            foreach($this->relatednames as $key => $value) { $this->relatednames[$key] = htmlspecialchars($this->relatednames[$value]); }
        }
        if(!is_array($this->urls)) {
            foreach($this->urls as $key => $value) { $this->urls[$key] = htmlspecialchars($this->urls[$value]); }
        }
    }
